...
from snagrecover.protocols.bcm import rom_code_send_file, BootcodeCommand, bootcode_get_command, bootcode_send_file_size, bootcode_send_file
		...
		for member_name in members_names:
			...
			if normpath(member_path) == normpath(member_name):
				if member_path != member_name:
					logger.debug(f"Tar member '{member_name}' match '{member_path}' using normalized path '{normpath(member_path)}'")
				return tar_file.getmember(member_name)

		_norm_path = f" ({normpath(member_path)})" if normpath(member_path) != member_path else ""
...
def tar_extract_file(tar_blob: bytes, file_path: str) -> bytes:
	...
	"""
	with BytesIO(tar_blob) as bootfiles_bin_file, tarfile.open(fileobj=bootfiles_bin_file) as tar_file:
		member = tar_getmember(tar_file, file_path)
		member = tar_file.extractfile(member)
		if member is None:
			raise ValueError(f"{file_path} exists in tar archive but is is not a file or link")
		file_bytes = member.read(-1)

	return file_bytes


def get_fileblob_from_bootfiles(tar_blob: bytes, file_name: str, file_paths: [str]) -> bytes:
		"""
		...
		if len(file_exist_paths) == 0:
			err_msg = f"'bootfiles' tar archive doesn't contain '{file_name}' ({file_paths})"
...
class PartitionEntry(BinFileHeader):
	...
	def get_fat_start_offset(self) -> int:
		...
		if self.is_fat_lba():
			...
			if self.is_active():
				raise ValueError(f"Partition type ({self._type:#04x}) is not FAT with LBA ([{','.join(f'{i:#04x}' for i in PartitionEntry.FAT_LBA_COMPATIBLE)}])")
			else:
				raise ValueError(f"Partition is not active (partition status: {self.status:#04x})")
...
def modify_boot_fw(boot_path: str, uboot_blob: bytes) -> None:
	...
	try:
		...
		logger.debug(f"First partition FAT byte offset's: {first_partition_offset:#x} ({first_partition_offset})")
	except ValueError as ve:
		logger.critical(ve)
		raise ve
	...
	with open_fs(f"fat://{boot_path}?offset={first_partition_offset}") as bootimgfs:
		...
		try:
			...
			logger.debug(f"Can't delete '{kernel_path}'from 'boot' firwmare: not found")
		...
		try:
			...
			err_msg = f"Error while trying to modify '{config_path}' from 'boot' firmware"
			logger.critical(err_msg)
			pfe.add_note(err_msg)
			raise pfe
...
def bcm_run(port, fw_name: str, fw_blob: bytes, subfw_name: str) -> None:
	...
	if subfw_name == "bootcode":
		...
		bootcode_blob = get_fileblob_from_bootfiles(tar_blob, bootcode_name, bootcode_paths)
	...
	elif subfw_name == "bootcode_firmwares": # Also handle firmwares 'boot' and 'u-boot'
		"""
		bootcode function as a file client. It issue requests (GET_FILE_SIZE, GET_FILE) that we must serve.
		...
		if should_modify_boot_fw():
			...
			logger.info("U-Boot found in recovery config, copying 'boot' firmware and update it")
			uboot_blob = load_fw("u-boot")
			modify_boot_fw(boot_fw_path_copy, uboot_blob)
		else:
			logger.debug("U-Boot not found in recovery config, use unmodified 'boot' firmware")
		...
		while command != BootcodeCommand.DONE:
			logger.debug(f"Serving command '{command!r}' on requested file '{file_name}'")
			...
			if file_name == previous_file_name:
				...
				if firmware_name == "boot" and should_modify_boot_fw():
					...
					requested_fw_blob = load_fw(firmware_name, check_fw=(file_name != "config.txt"))
				logger.debug(f"'Requested file is '{file_name}', using firmware '{firmware_name}' ({file_path})")
			else:
				file_paths = [f"{subfolder_name}/{file_name}", file_name]
				requested_fw_blob = get_fileblob_from_bootfiles(tar_blob, file_name, file_paths)
